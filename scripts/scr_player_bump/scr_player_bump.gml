function scr_player_bump()
{
	...
	if (floor(image_index) == (image_number - 1) && character == characters.pizzelle && sprite_index != spr_player_suplexbump)
		state = states.normal;
	if (grounded && (character == characters.pizzano || sprite_index == spr_player_suplexbump) && (sprite_index != spr_player_wallsplat))
		state = states.normal;
	    if ((sprite_index == spr_player_wallsplat) && (vsp > 0))
        vsp = 0
	if ((sprite_index != spr_player_catched && (sprite_index != spr_tumbleend && sprite_index != spr_player_mach3hitwall)) && sprite_index != spr_pizzano_shoulderbash && sprite_index != spr_player_suplexbump && (sprite_index != spr_player_wallsplat))
